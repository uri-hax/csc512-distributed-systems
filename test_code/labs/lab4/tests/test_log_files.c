...
// Checks the existence of log files for each thread
// Previously expected all 10 files to exist
MU_TEST(test_thread_log_files_exists) {
    char error_msg[MAX_PATH_LEN + 32];
    int num_threads = 0;

    // Determine the number of threads by checking which files exist
